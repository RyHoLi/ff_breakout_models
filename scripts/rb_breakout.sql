...
select
	...
	, CASE WHEN t1.tgt_sh::DECIMAL(5,2) IS NULL THEN 0 ELSE t1.tgt_sh::DECIMAL(5,2) END AS tgt_sh
 	, CASE WHEN t1.ay_sh::DECIMAL(5,2) IS NULL THEN 0 ELSE t1.ay_sh::DECIMAL(5,2) END AS ay_sh
 	, case when t1.yac_sh = 'infinity' then 0 else t1.yac_sh end as yac_sh
...
JOIN (SELECT player_id, season, COUNT(*) as gp FROM nfl_weekly WHERE carries >= 1 GROUP BY 1,2) c 
ON (a.player_id = c.player_id AND a.season = c.season )
WHERE b.position = 'RB') t1
LEFT JOIN 
(select
	a.player_id
	, b.name
